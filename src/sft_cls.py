...
PROMPT_REL = """Given an image and a set of detected objects ```[{OBJECTS}]```, identify all possible visual relationships between two objects.
### **Guidelines:**
- **Objects:**
  - Objects are formatted as `"object_name.number"` (e.g., `"person.1"`, `"cat.2"`).

- **Relationships:**
  - Represent interactions accurately using `"subject"`,  `"object"`, and `"predicate"`.
...
{{
  "relationships": [
    {{"subject": "person.1", "object": "bike.2", "predicate": "riding"}},
    {{"subject": "person.1", "object": "helme.3", "predicate": "wearing"}}
  ]
}}
```
...
def format_answer(objects:str, relationships:str, shuffle=False):
    ...
    return objects, structured_answer
...
def format_data(sample, use_predefined_cats=False, remove_image_size_in_prompt=True, shuffle=False):
    ...
    objs, answer = format_answer(objs, sample["relationships"], shuffle=shuffle)
    prompt = PROMPT_REL.format(OBJECTS=objs)
