...
# Ensure the number of nodes is divisible by 3
...
# Training GPUs: NUM_VLLM_NODES with [0,1,2,3],  and x = (${GROUP_SIZE} - 4)*NUM_VLLM_NODES // 8 nodes with [0-7]
# e.g., NUM_VLLM_NODES=2, GROUP_SIZE=8, x = (8-4)*2//8=1, that means, training GPUs contain 2 nodes with GPU rank[0,1,2,3], and x=1 node with GPU rank[0-7]

MIXED_NODES=16  # nodes 0-15: mixed; nodes 16...: training only
GPUS_PER_NODE=8
...
# ---------- Create IP:PORT File for vLLM servers (only on mixed nodes) ----------
...
# ---------- Launch Jobs on Each Node ----------
for i in "${!NODELIST[@]}"; do
    node=${NODELIST[$i]}

    if [ $i -lt ${MIXED_NODES} ]; then
        # Mixed node: launch both vLLM and training
        ...
        # Pure training node: launch training on all GPUs 0-7
        srun --nodes=1 --ntasks=1 -w $node bash -c "
            export NODE_RANK=$i
            export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
            echo \"Launching training on $node (GPUs 0-7)\"
            torchrun --nnodes ${NUM_NODES} --nproc_per_node 8 \
                --node_rank \$NODE_RANK \
                --rdzv_id grpo_run \
                --rdzv_backend c10d \
                --rdzv_endpoint ${HEAD_NODE_IP}:${RDZV_PORT} \
